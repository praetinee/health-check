...
if "person" in st.session_state:
    ...
    def shorten_eye_advice(text: str) -> str:
        ...
        if "บริหารสายตา" in text and "พักสายตา" in text:
            return "พัก-บริหารสายตา ตรวจปีละครั้ง"
        if "เครื่องจักร" in text or "ขับรถ" in text:
            return "ระวังขับรถ/ใกล้เครื่องจักร พักสายตา"
        if "ควรพบแพทย์" in text:
            return "พบแพทย์ตรวจสายตา"
        if "เหมาะสมกับลักษณะงาน" in text:
            return "เหมาะกับลักษณะงาน"
        if "เหมาะสมกับงาน" in text:
            return "เหมาะกับงาน"
    ...
    for y in eye_years:
        ...
        for field, prefixes in eye_metrics.items():
            ...
            if field == "คำแนะนำ":
                value = shorten_eye_advice(value)
